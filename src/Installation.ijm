...
//Create the shortcut in IJ macro menu for the first installation (Table analysis)
PCommandLine = PathFolderInput+ "Startup_CL2.txt";
SUM = File.openAsString(PathSUM);
pos =lastIndexOf(SUM, "//End_Nuke-Break_Table");
if(pos == -1){
	SUM = SUM + "\n\n" + File.openAsString(PCommandLine);
	Startup = File.open(PathSUM);
	print(Startup, SUM);
